...
def run_(request, link):
    ...
    if link == "TAMERmountain":
        template = "experiment/run_tamer.html"
    else:
        template = "experiment/run.html"
    return render(request, template, {"room_name": room_name, 'ws_setting': WS_SETTING, 'inputsListened': experiment.input_list, 'experiment_name': experiment.name, 'experiment_type': experiment.type, 'experiment_train': experiment.train})


@login_required
def run_tamer_(request, link):
    # Create empty config form
    form = ConfigForm()
    # Check if all the fields have been saved in the session
    saved = all((k in request.session.keys() or not form.fields[k].required) for k in form.fields.keys())
    ...
    experiment = models.Experiment.objects.get(link=link)
    room_name = request.session['room_name']
    return render(request, "experiment/run_tamer.html", {"room_name": room_name, 'ws_setting': WS_SETTING, 'inputsListened': experiment.input_list, 'experiment_name': experiment.name, 'experiment_type': experiment.type, 'experiment_train': experiment.train})
